...
namespace vke
{
    ...
    struct DescriptorAllocatorHandle
    {
            ...
            DescriptorAllocatorHandle& operator=(
                const DescriptorAllocatorHandle&) = delete;
            ...
            bool Allocate(const VkDescriptorSetLayout& layout,
    ...
    class DescriptorAllocatorPool
    {
        public:
            ...
            static DescriptorAllocatorPool* Create(const VkDevice& device,
                                                   int nFrames = 3);

            // not thread safe
            // switches default allocators to the next frame. When frames loop
            // it will reset the descriptors of that frame
            virtual void Flip() = 0;

            // not thread safe
            // override the pool size for a specific descriptor type. This will
            // be used new pools are allocated
            virtual void SetPoolSizeMultiplier(VkDescriptorType type,
                                               float multiplier) = 0;
