...
namespace FooGame
{
    ...
    std::shared_ptr<Mesh> LoadGLTF(const std::string& path, bool isGlb)
    {
                    ...
                    primitive.attributes.end())
                {
                    colAccessor =
                        &model.accessors[primitive.attributes.find("COLOR_0")
                                             ->second];
                    colBufferView = &model.bufferViews[colAccessor->bufferView];
                    colBuffer     = &model.buffers[colBufferView->buffer];
        ...
        return std::make_shared<Mesh>(std::move(vertices), std::move(indexes));
    }
}  // namespace FooGame
