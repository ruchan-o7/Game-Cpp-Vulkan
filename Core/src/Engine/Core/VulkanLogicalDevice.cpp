...
namespace ENGINE_NAMESPACE
{
    ...
    VulkanLogicalDevice::VulkanLogicalDevice(const VulkanPhysicalDevice& physicalDevice,
                                             ...
                                             const VkAllocationCallbacks* vkAllocator)
        : m_VkAllocator{vkAllocator}, m_EnabledFeatures{*DeviceCI.pEnabledFeatures}
    {
        VK_CALL(vkCreateDevice(physicalDevice.GetVkDeviceHandle(), &DeviceCI, vkAllocator,
                               &m_VkDevice));
        ...
        auto ComputeAccessMask = VK_ACCESS_INDIRECT_COMMAND_READ_BIT | VK_ACCESS_UNIFORM_READ_BIT |
                                 VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
        auto TransferAccessMask = VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT |
        ...
        {
            GraphicsStages |= VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT |
                              VK_PIPELINE_STAGE_TESSELLATION_EVALUATION_SHADER_BIT;
        }
    ...
    VkQueue VulkanLogicalDevice::GetQueue(uint32_t queueFamilyIndex, uint32_t queueIndex)
    {
        ...
        vkGetDeviceQueue(m_VkDevice,
                         ...
                         &vkQueue);
        assert(vkQueue != VK_NULL_HANDLE);
        return vkQueue;
    }
    void VulkanLogicalDevice::WaitIdle() const
    {
        auto err = vkDeviceWaitIdle(m_VkDevice);
        assert(err == VK_SUCCESS && "Failed to idle device");
        (void)err;
    }

    template <typename VkObjectType, VulkanHandleTypeId VkTypeId, typename VkCreateObjectFuncType,
              typename VkObjectCreateInfoType>
    VulkanObjectWrapper<VkObjectType, VkTypeId> VulkanLogicalDevice::CreateVulkanObject(
        ...
        const char* DebugName, const char* ObjectType) const
    {
        ...
        if (err != VK_SUCCESS)
        {
            FOO_ENGINE_ERROR("Failed to create Vulkan {0} : {1}", ObjectType, DebugName);
        }
    ...
    CommandPoolWrapper VulkanLogicalDevice::CreateCommandPool(
        const VkCommandPoolCreateInfo& CmdPoolCI, const char* DebugName) const
    {
        assert(CmdPoolCI.sType == VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO);
        return CreateVulkanObject<VkCommandPool, VulkanHandleTypeId::CommandPool>(
            vkCreateCommandPool, CmdPoolCI, DebugName, "command pool");
    }

    BufferWrapper VulkanLogicalDevice::CreateBuffer(const VkBufferCreateInfo& BufferCI,
                                                    const char* DebugName) const
    {
        assert(BufferCI.sType == VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO);
        return CreateVulkanObject<VkBuffer, VulkanHandleTypeId::Buffer>(vkCreateBuffer, BufferCI,
                                                                        DebugName, "buffer");
    }

    BufferViewWrapper VulkanLogicalDevice::CreateBufferView(
        const VkBufferViewCreateInfo& BuffViewCI, const char* DebugName) const
    {
        assert(BuffViewCI.sType == VK_STRUCTURE_TYPE_BUFFER_VIEW_CREATE_INFO);
        return CreateVulkanObject<VkBufferView, VulkanHandleTypeId::BufferView>(
    ...
    ImageWrapper VulkanLogicalDevice::CreateImage(const VkImageCreateInfo& ImageCI,
                                                  const char* DebugName) const
    {
        assert(ImageCI.sType == VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO);
        return CreateVulkanObject<VkImage, VulkanHandleTypeId::Image>(vkCreateImage, ImageCI,
                                                                      DebugName, "image");
    }
    ...
    ImageViewWrapper VulkanLogicalDevice::CreateImageView(const VkImageViewCreateInfo& ImageViewCI,
                                                          const char* DebugName) const
    {
        assert(ImageViewCI.sType == VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO);
        return CreateVulkanObject<VkImageView, VulkanHandleTypeId::ImageView>(
            vkCreateImageView, ImageViewCI, DebugName, "image view");
    }

    SamplerWrapper VulkanLogicalDevice::CreateSampler(const VkSamplerCreateInfo& SamplerCI,
                                                      const char* DebugName) const
    {
        assert(SamplerCI.sType == VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO);
        return CreateVulkanObject<VkSampler, VulkanHandleTypeId::Sampler>(
            vkCreateSampler, SamplerCI, DebugName, "sampler");
    }

    FenceWrapper VulkanLogicalDevice::CreateFence(const VkFenceCreateInfo& FenceCI,
                                                  const char* DebugName) const
    {
        assert(FenceCI.sType == VK_STRUCTURE_TYPE_FENCE_CREATE_INFO);
        return CreateVulkanObject<VkFence, VulkanHandleTypeId::Fence>(vkCreateFence, FenceCI,
                                                                      DebugName, "fence");
    }

    RenderPassWrapper VulkanLogicalDevice::CreateRenderPass(
        const VkRenderPassCreateInfo& RenderPassCI, const char* DebugName) const
    {
        assert(RenderPassCI.sType == VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO);
        return CreateVulkanObject<VkRenderPass, VulkanHandleTypeId::RenderPass>(
            vkCreateRenderPass, RenderPassCI, DebugName, "render pass");
    }

    RenderPassWrapper VulkanLogicalDevice::CreateRenderPass(
        const VkRenderPassCreateInfo2& RenderPassCI, const char* DebugName) const
    {
    ...
    PipelineWrapper VulkanLogicalDevice::CreateComputePipeline(
        const VkComputePipelineCreateInfo& PipelineCI, VkPipelineCache cache,
        const char* DebugName) const
    {
        assert(PipelineCI.sType == VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO);
        ...
        auto err =
            vkCreateComputePipelines(m_VkDevice, cache, 1, &PipelineCI, m_VkAllocator, &vkPipeline);
        FOO_ENGINE_ERROR("Failed to create compute pipeline : {0}", DebugName);
    ...
    PipelineWrapper VulkanLogicalDevice::CreateGraphicsPipeline(
        const VkGraphicsPipelineCreateInfo& PipelineCI, VkPipelineCache cache,
        const char* DebugName) const
    {
        assert(PipelineCI.sType == VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO);
    ...
    PipelineWrapper VulkanLogicalDevice::CreateRayTracingPipeline(
        const VkRayTracingPipelineCreateInfoKHR& PipelineCI, VkPipelineCache cache,
        const char* DebugName) const
    {
#if DILIGENT_USE_VOLK
        assert(PipelineCI.sType == VK_STRUCTURE_TYPE_RAY_TRACING_PIPELINE_CREATE_INFO_KHR);
        ...
        auto err = vkCreateRayTracingPipelinesKHR(m_VkDevice, VK_NULL_HANDLE, cache, 1, &PipelineCI,
                                                  m_VkAllocator, &vkPipeline);
        FOO_ENGINE_ERROR("Failed to create ray tracing pipeline  : {0}", DebugName);
        ...
        return PipelineWrapper{GetSharedPtr(), std::move(vkPipeline)};
#else
        assert(0 && "vkCreateRayTracingPipelinesKHR is only available through Volk");
    ...
    {
        assert(ShaderModuleCI.sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO);
        return CreateVulkanObject<VkShaderModule, VulkanHandleTypeId::ShaderModule>(
    ...
    PipelineLayoutWrapper VulkanLogicalDevice::CreatePipelineLayout(
        const VkPipelineLayoutCreateInfo& PipelineLayoutCI, const char* DebugName) const
    {
        assert(PipelineLayoutCI.sType == VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO);
        return CreateVulkanObject<VkPipelineLayout, VulkanHandleTypeId::PipelineLayout>(
            vkCreatePipelineLayout, PipelineLayoutCI, DebugName, "pipeline layout");
    }

    FramebufferWrapper VulkanLogicalDevice::CreateFramebuffer(
        const VkFramebufferCreateInfo& FramebufferCI, const char* DebugName) const
    {
        assert(FramebufferCI.sType == VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO);
        return CreateVulkanObject<VkFramebuffer, VulkanHandleTypeId::Framebuffer>(
            vkCreateFramebuffer, FramebufferCI, DebugName, "framebuffer");
    }
    ...
    {
        assert(DescrPoolCI.sType == VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO);
        return CreateVulkanObject<VkDescriptorPool, VulkanHandleTypeId::DescriptorPool>(
    ...
    {
        assert(LayoutCI.sType == VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO);
        return CreateVulkanObject<VkDescriptorSetLayout, VulkanHandleTypeId::DescriptorSetLayout>(
            vkCreateDescriptorSetLayout, LayoutCI, DebugName, "descriptor set layout");
    }

    SemaphoreWrapper VulkanLogicalDevice::CreateVulkanSemaphore(
        const VkSemaphoreCreateInfo& SemaphoreCI, const char* DebugName) const
    {
        assert(SemaphoreCI.sType == VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO);
        return CreateVulkanObject<VkSemaphore, VulkanHandleTypeId::Semaphore>(
            vkCreateSemaphore, SemaphoreCI, DebugName, "semaphore");
    }

    SemaphoreWrapper VulkanLogicalDevice::CreateTimelineSemaphore(uint64_t InitialValue,
                                                                  const char* DebugName) const
    {
        // assert(m_EnabledExtFeatures.TimelineSemaphore.timelineSemaphore == VK_TRUE);
        ...
        return CreateVulkanObject<VkSemaphore, VulkanHandleTypeId::Semaphore>(
            vkCreateSemaphore, SemaphoreCI, DebugName, "timeline semaphore");
    }

    QueryPoolWrapper VulkanLogicalDevice::CreateQueryPool(const VkQueryPoolCreateInfo& QueryPoolCI,
                                                          const char* DebugName) const
    {
        assert(QueryPoolCI.sType == VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO);
        return CreateVulkanObject<VkQueryPool, VulkanHandleTypeId::QueryPool>(
            vkCreateQueryPool, QueryPoolCI, DebugName, "query pool");
    }

    AccelStructWrapper VulkanLogicalDevice::CreateAccelStruct(
        const VkAccelerationStructureCreateInfoKHR& CI, const char* DebugName) const
    {
#if DILIGENT_USE_VOLK
        assert(CI.sType == VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR);
        return CreateVulkanObject<VkAccelerationStructureKHR,
                                  VulkanHandleTypeId::AccelerationStructureKHR>(
            vkCreateAccelerationStructureKHR, CI, DebugName, "acceleration structure");
#else
        assert(0 && "vkCreateAccelerationStructureKHR is only available through Volk");
        return AccelStructWrapper{};
#endif
    }

    VkCommandBuffer VulkanLogicalDevice::AllocateVkCommandBuffer(
        const VkCommandBufferAllocateInfo& AllocInfo, const char* DebugName) const
    {
        assert(AllocInfo.sType == VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO);
        ...
        if (err != VK_SUCCESS)
        {
            FOO_ENGINE_ERROR("Failed to allocate command buffer {0}", DebugName);
            return {};
        }
        (void)err;
    ...
    VkDescriptorSet VulkanLogicalDevice::AllocateVkDescriptorSet(
        const VkDescriptorSetAllocateInfo& AllocInfo, const char* DebugName) const
    {
        assert(AllocInfo.sType == VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO);
    ...
    PipelineCacheWrapper VulkanLogicalDevice::CreatePipelineCache(
        const VkPipelineCacheCreateInfo& CI, const char* DebugName) const
    {
        assert(CI.sType == VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO);
        return CreateVulkanObject<VkPipelineCache, VulkanHandleTypeId::PipelineCache>(
            vkCreatePipelineCache, CI, DebugName, "pipeline cache");
    }
    ...
    void VulkanLogicalDevice::ReleaseVulkanObject(
        DescriptorSetLayoutWrapper&& DescriptorSetLayout) const
    {
...
#else
        assert(0 && "vkDestroyAccelerationStructureKHR is only available through Volk");
#endif
    }
    ...
    {
        assert(Pool != VK_NULL_HANDLE && Set != VK_NULL_HANDLE);
    ...
    {
        assert(Pool != VK_NULL_HANDLE && CmdBuffer != VK_NULL_HANDLE);
    ...
    VkResult VulkanLogicalDevice::BindBufferMemory(VkBuffer buffer, VkDeviceMemory memory,
                                                   VkDeviceSize memoryOffset) const
    {
        return vkBindBufferMemory(m_VkDevice, buffer, memory, memoryOffset);
    }

    VkResult VulkanLogicalDevice::BindImageMemory(VkImage image, VkDeviceMemory memory,
                                                  VkDeviceSize memoryOffset) const
    {
        return vkBindImageMemory(m_VkDevice, image, memory, memoryOffset);
    }

    VkDeviceAddress VulkanLogicalDevice::GetAccelerationStructureDeviceAddress(
        VkAccelerationStructureKHR AS) const
    {
#if DILIGENT_USE_VOLK
...
#else
        assert(0 && "vkGetAccelerationStructureDeviceAddressKHR is only available through Volk");
        return VK_ERROR_FEATURE_NOT_PRESENT;
#endif
    }

    void VulkanLogicalDevice::GetAccelerationStructureBuildSizes(
        const VkAccelerationStructureBuildGeometryInfoKHR& BuildInfo,
        const uint32_t* pMaxPrimitiveCounts,
        VkAccelerationStructureBuildSizesInfoKHR& SizeInfo) const
    {
#if DILIGENT_USE_VOLK
        vkGetAccelerationStructureBuildSizesKHR(m_VkDevice,
                                                VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                                &BuildInfo, pMaxPrimitiveCounts, &SizeInfo);
#else
        assert(0 && "vkGetAccelerationStructureBuildSizesKHR is only available through Volk");
#endif
    }

    VkResult VulkanLogicalDevice::MapMemory(VkDeviceMemory memory, VkDeviceSize offset,
    ...
    VkResult VulkanLogicalDevice::InvalidateMappedMemoryRanges(
        uint32_t memoryRangeCount, const VkMappedMemoryRange* pMemoryRanges) const
    {
    ...
    VkResult VulkanLogicalDevice::WaitForFences(uint32_t fenceCount, const VkFence* pFences,
                                                VkBool32 waitAll, uint64_t timeout) const
    {
        return vkWaitForFences(m_VkDevice, fenceCount, pFences, waitAll, timeout);
    }

    VkResult VulkanLogicalDevice::GetSemaphoreCounter(VkSemaphore TimelineSemaphore,
                                                      uint64_t* pSemaphoreValue) const
    {
#if DILIGENT_USE_VOLK
        return vkGetSemaphoreCounterValueKHR(m_VkDevice, TimelineSemaphore, pSemaphoreValue);
#else
        assert(0 && "vkGetSemaphoreCounterValueKHR is only available through Volk");
        return VK_ERROR_FEATURE_NOT_PRESENT;
#endif
    }
    ...
    {
#if DILIGENT_USE_VOLK
        assert(SignalInfo.sType == VK_STRUCTURE_TYPE_SEMAPHORE_SIGNAL_INFO);
        return vkSignalSemaphoreKHR(m_VkDevice, &SignalInfo);
#else
        assert(0 && "vkSignalSemaphoreKHR is only available through Volk");
        return VK_ERROR_FEATURE_NOT_PRESENT;
#endif
    }

    VkResult VulkanLogicalDevice::WaitSemaphores(const VkSemaphoreWaitInfo& WaitInfo,
                                                 uint64_t Timeout) const
    {
#if DILIGENT_USE_VOLK
        assert(WaitInfo.sType == VK_STRUCTURE_TYPE_SEMAPHORE_WAIT_INFO);
        return vkWaitSemaphoresKHR(m_VkDevice, &WaitInfo, Timeout);
#else
        assert(0 && "vkWaitSemaphoresKHR is only available through Volk");
        return VK_ERROR_FEATURE_NOT_PRESENT;
#endif
    }

    void VulkanLogicalDevice::UpdateDescriptorSets(
        uint32_t descriptorWriteCount, const VkWriteDescriptorSet* pDescriptorWrites,
        uint32_t descriptorCopyCount, const VkCopyDescriptorSet* pDescriptorCopies) const
    {
        vkUpdateDescriptorSets(m_VkDevice, descriptorWriteCount, pDescriptorWrites,
                                                   ...
                                                   VkCommandPoolResetFlags flags) const
    {
        ...
        assert(err == VK_SUCCESS && "Failed to reset command pool");
        return err;
    }
                                                      ...
                                                      VkDescriptorPoolResetFlags flags) const
    {
        ...
        assert(err == VK_SUCCESS && "Failed to reset descriptor pool");
        return err;
    }

    void VulkanLogicalDevice::ResetQueryPool(VkQueryPool queryPool, uint32_t firstQuery,
                                             uint32_t queryCount) const
    {
#if DILIGENT_USE_VOLK
        vkResetQueryPoolEXT(m_VkDevice, queryPool, firstQuery, queryCount);
#else
        assert(0 && "Host query reset is not supported when vulkan library is linked statically");
#endif
    }
}  // namespace ENGINE_NAMESPACE
