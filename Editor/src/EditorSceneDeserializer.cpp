  ...
  std::unique_ptr<EditorScene> EditorSceneDeserializer::DeSerialize(
	...
	for (auto& asset : data["assets"]) {
	  ...
	  if (assetJson["format"] == "obj") {
		meshData.ModelPtr = AssetLoader::LoadObjModel(assetAbsPath.string());
		...
		meshData.ModelPtr->Name = assetJson["name"];
		for (const auto& img : assetJson["textures"]) {
		  auto textureAbsPath = assetRelativePath.parent_path() / img["path"];
